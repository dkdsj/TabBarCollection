...
//红拂夜奔 -> ***奔
- (NSString *)na_stringStarOverLastStrNum:(NSInteger)num {
    
    NSMutableString *strNew = [NSMutableString stringWithCapacity:self.length];
    for (int i = 0; i < self.length; i++) {
        [strNew appendString:@"*"];
    }
    
    NSString *lastStr = [self substringFromIndex:self.length-num];
    
    return  [strNew stringByReplacingCharactersInRange:NSMakeRange(self.length-num, num) withString:lastStr];
}
